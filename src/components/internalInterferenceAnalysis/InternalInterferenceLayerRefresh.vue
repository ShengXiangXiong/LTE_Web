  ...
  <div>
    <div class="floatPage">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="170px">
        ...
        <div class="sub_css">
          <el-form-item>
            <el-button type="primary" @click="submitForm('ruleForm')">网内干扰分析图层刷新</el-button>
...
<script>
  import {
    PostRefreshExcessiveLayer,
    PostRefreshOverlappedLayer,
    PostRefreshPCIconflictLayer,
    PostRefreshPCIconfusionLayer,
    PostRefreshPCImod3Layer,
    PostRefreshWeakLayer
  } from '@/httpConfig/api'

  export default {
    name: 'InternalInterferenceLayerRefresh',
    data () {
      return {
        ruleForm: {
    ...
    methods: {
      submitForm (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            PostRefreshWeakLayer(this.ruleForm).catch(err => {
              errInfo = err
            })
              .then(response => {
            ...
            PostRefreshExcessiveLayer(this.ruleForm).catch(err => {
              errInfo = err
            })
              .then(response => {
                if (response && response.data.ok) {
            ...
            PostRefreshOverlappedLayer(this.ruleForm).catch(err => {
              errInfo = err
            })
              .then(response => {
                if (response && response.data.ok) {
            ...
            PostRefreshPCIconflictLayer(this.ruleForm).catch(err => {
              errInfo = err
            })
              .then(response => {
                if (response && response.data.ok) {
            ...
            PostRefreshPCIconfusionLayer(this.ruleForm).catch(err => {
              errInfo = err
            })
              .then(response => {
                if (response && response.data.ok) {
            ...
            PostRefreshPCImod3Layer(this.ruleForm).catch(err => {
              errInfo = err
            })
              .then(response => {
                if (response && response.data.ok) {
